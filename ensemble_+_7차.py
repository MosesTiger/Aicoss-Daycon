...
Original file is located at
    https://colab.research.google.com/drive/1LwKF3rIL4lzszGzz2Zvh3LG0WGPSy8XQ
"""
...
model_list = [
    ModifiedBaseModel(num_classes=60),  # ResNet101 기반 모델
]

densenet121 = models.densenet121(pretrained=True)
num_ftrs = densenet121.classifier.in_features
densenet121.classifier = nn.Linear(num_ftrs, 60)  # 마지막 분류기 레이어 변경
model_list.append(densenet121)

# EfficientNetB0 수정
def get_state_dict(self, *args, **kwargs):
    kwargs.pop("check_hash", None)  # check_hash 인자를 제거
    return load_state_dict_from_url(self.url, *args, **kwargs)

EfficientNet_B0_Weights.get_state_dict = get_state_dict

# EfficientNet-B0 모델을 불러오기
weights = EfficientNet_B0_Weights.IMAGENET1K_V1
efficientnetb0 = models.efficientnet_b0(weights=weights)

# 두 번째 코드 조각
num_ftrs = efficientnetb0.classifier[1].in_features
efficientnetb0.classifier[1] = nn.Linear(num_ftrs, 60)  # 마지막 분류기 레이어 변경

# 모델 리스트에 추가
model_list.append(efficientnetb0)

class EnsembleModel(nn.Module):
    def __init__(self, model_list, num_classes=60):
        super(EnsembleModel, self).__init__()
        self.model_list = nn.ModuleList(model_list)
        self.num_classes = num_classes

    def forward(self, x):
        # 각 모델의 예측값을 평균내어 최종 예측을 생성
        outputs = [model(x) for model in self.model_list]
        avg_output = torch.mean(torch.stack(outputs), dim=0)
        return avg_output

# 앙상블 모델 생성
ensemble_model = EnsembleModel(model_list=model_list)
...
def train(model, optimizer, train_loader, val_loader, device):
    ...
    # FocalLoss의 인스턴스로 criterion을 변경합니다.
...
model = EnsembleModel(num_classes=60, model_list=model_list)
...
optimizer = torch.optim.Adam(params = model.parameters(), lr = CFG["LEARNING_RATE"])
infer_model2 = train(infer_model,optimizer, train_loader, val_loader,device)
...
optimizer = torch.optim.Adam(params = model.parameters(), lr = CFG["LEARNING_RATE"])
infer_model3 = train(infer_model2,optimizer, train_loader, val_loader,device)
